...
Prorab sets the `dot_so` variable to the suffix for shared lybrary files which is native for the host system.
...
Prorab sets the `dot_exe` variable to the suffix for executable files which is native for the host system.
...
- `this_headers_dir` - root directory of the header files which are to be installed with the library. Relative to `$(d)`. The directory is searched recursively for C (`.h`) and C++ header files (`$(this_dot_hxx)`). The `install` target will install these header files to `$(DESTDIR)$(PREFIX)/include` preserving directory structure. Also, the `test` target will test these headers for compilation, same as headers from `this_hdrs`, but with no compilation flags, except `-std=...` filtered from `$(this_cflags)`/`$(this_cxxflags)`.
