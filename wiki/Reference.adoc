...
Some variables, instead of clearing, are set to their default values. These variables are:
- `this_dot_cxx`
- `this_dot_hxx`
- `this_cc`
- `this_cxx`
...
Note, that `prorab.mk` already have a call to this macro at its end. So, right after including `prorab.mk` one can be sure that all `this_`-prefixed variables are empty.
