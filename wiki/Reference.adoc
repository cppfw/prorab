...
.generated targets
- `$(this_name)$(dot_exe)` - link executable. This target is added to `all` target dependencies.
...
- `$(prorab_this_obj_dir)$(prorab_this_obj_spacer)%.o: $(d)%` - compile source files to object files.
